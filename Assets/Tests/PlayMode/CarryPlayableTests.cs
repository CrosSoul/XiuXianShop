...
namespace XiuXianShop.Tests
{
    ...
    {
        [UnityTest,Category("DP31"),Category("DP27")] public IEnumerator NoPortableChoiceStillCarriesTwoDifferentShapesAndReopensWithoutCopying()
        {
            ...
            yield return CloseBusinessForOuting();yield return Click("CarryOpen");Assert.That(shop.FindButton("CarryNone"),Is.Not.Null);
            ...
            yield return Click("CarryHide");yield return CloseBusinessForOuting();yield return Click("CarryOpen");Assert.That(s.Items.Count,Is.EqualTo(2));
        ...
        [UnityTest,Category("DP31"),Category("DP27")] public IEnumerator MultipleActualPacksCancelWithoutChangesAndChosenContentsMoveBetweenHandsAndPack()
        {
            ...
            yield return Drag(herb,ContainerId.Interior,0,0);yield return Click("StorageClose");
            var before=s.Items.Select(i=>(i,i.Container,i.StorageItemId,i.X,i.Y,i.Rotation,i.Flipped)).ToArray();
            yield return CloseBusinessForOuting();yield return Click("CarryOpen");Assert.That(shop.GetComponentsInChildren<UnityEngine.UI.Button>().Count(b=>b.name.StartsWith("CarryOption_")),Is.EqualTo(2));
            yield return Click("CarryOption_"+packs[1].Id);yield return Click("CarryCancel");
            Assert.That(s.Items.Select(i=>(i,i.Container,i.StorageItemId,i.X,i.Y,i.Rotation,i.Flipped)),Is.EqualTo(before));
            Assert.That(s.IsCarrying,Is.False);
            yield return CloseBusinessForOuting();yield return Click("CarryOpen");yield return Click("CarryOption_"+packs[1].Id);yield return Click("CarryConfirm");
            ...
            yield return Click("CarryHide");yield return CloseBusinessForOuting();yield return Click("CarryOpen");Assert.That(s.Items.Count,Is.EqualTo(4));
