...
- 基线8c6e4cb，开工工作区干净。按任务读取DP-60、DP-58/59最新验收、DP-10、G-06 v12、G-04 v16 AC15/43/63—76、G01/G03 v8、G07/G09 v5。仅因设备占格/价值缺失，定向查物品目录的device_alchemy_furnace候选行：ProductionEquipment、3×3、240，授权仅作灰盒参考；不导入完整目录、不改正式数据。
- 复用ShopSession.Alchemy内核，增加真实设备ID/店内上下文与备料区域，不复制计时、品相、研磨、火候或供能逻辑。AlchemySettings.shopStaminaCost默认20；只在所有开炉检查通过后扣一次。店内仅Closed阶段允许操作，20→0成功、19拒绝；完成/中止后手动准备下一炉，输出未取走则拒绝。异地仍是一访问一炉、仅收地点体力。
- 当前Session开发菜单新增Grant Miniature Alchemy Furnace (no reset)，按仓库空间发放真实炉实例；三方材料沿用DP59补包，先打开包把材料/灵石手动取到仓库根层再打开设备。旧默认“炼制回气丹”按钮改为打开仓库真实炉的快捷入口，不再允许默认Play绕过设备/供能/体力瞬间合成；历史Craft方法仅保留给既有隔离逻辑测试。
- 共用Alchemy操作页与Item/Grid/Tooltip，店内下方显示实际仓库根层，产物先到收丹位再手动回仓；不从容器自动抽料。店内页打开时暂不操作储存弹窗，关闭前须收回全部设施物品；运行/研磨中拒绝关闭、设备移动、外出与跨月，预置首味后须开炉或明确中止。未实现并行设备或跨回合在制品，保存沿用开发Temp路径与既有品相保存限制。
- 最终验证：Edit Mode173/173（新增DP60八项）；DP60 Play4/4，含三丹方、默认Session开发注入、真实设备点击/仓库拖料/产物回仓、教学方连续两炉、Tooltip、跨月再营业、原资产不变与无设备快捷入口不合成。相关Play回归：DP27分类19/19（含异地炼丹/外出/灵石），DP59连续会话3/3，DP45体力1/1，储存拖动与保存恢复1/1，合计28/28。原始结果集中在Evidence/DP60/editmode.json与playmode.json。额度中断和筛选返回0项均未计通过。
- 正常Play使用当前Session发炉/材料并打开设备，检查实际画面；修正备料标题重叠、店内结束后误用“一访问一炉”的提示。最后两项仅为显示修正，重新编译通过；未重复全套玩法测试。临时截图已通过Unity API清理，不保留截图合集。最终MCP Console Error0/Warning0，Editor ready/stopped，无编译或Domain Reload；未主动清日志。
- 人工复验：直接ShopPrototype Play，先用现有储物匣腾出仓库3×3 → Current Play Session发放微缩炉及一张丹方材料包 → 打开包把本炉材料/中品灵石取到仓库根层 → 关闭包，点击炉。营业前/中不能开炉；闭店后备料、装灵石、选丹方、预置首味并开炉。回气丹约8秒投灵露、16秒收丹，手动拖产物回仓；点“准备下一炉”，材料足够可再炼并再扣20体力。结束后收回设施物品、关闭炉、推进月份并营业。也请检查19点与20点临界及中止后的资源不返还。
- 待人工决定：每炉20体力、设备3×3/价值240，以及原有8秒节奏、宽容窗口、耗能/品相倍率；均非最终平衡。关闭前手动清空设施与根层取料的便利性需要试玩反馈。无需手动挂组件或配置资产；未改Scene/Prefab/Package/Project Settings，meta由Unity生成。未做Player/平台或玩家手感验收，不制作重复交接文档；当前仅本地未提交。
