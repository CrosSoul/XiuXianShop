...
当前交付：第一阶段原型已实现六项主目标，20 项自动测试通过。下一步是按 FIRST_PLAYABLE_GUIDE.md 人工试玩并修复体验问题；以下基础整理清单与建议顺序保留为历史规划，不代表功能仍未实现。
...
- 当前已有 ShopPrototype 可玩场景。16 项数据测试和 4 项实际 UI Play Mode 自动测试通过；人工试玩尚未执行，不能用 Editor ready 替代。
