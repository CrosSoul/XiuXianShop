...
当前交付：原型已完成类别/预算交易、全阶段自由搬动及开门快照；客流进一步改为每日固定五位、逐位随机买卖方向、最高展示价值类别生效、离散预算档位 ±10% 浮动。46 项自动测试通过。下一步按 FIRST_PLAYABLE_GUIDE.md 亲自体验客流与档位反馈；以下基础整理清单与建议顺序保留为历史规划，不代表功能仍未实现。
...
- 当前已有 ShopPrototype 可玩场景。39 项 Edit Mode 数据测试和 7 项实际 UI Play Mode 自动测试通过；人工试玩尚未执行，不能用 Editor ready 替代。
