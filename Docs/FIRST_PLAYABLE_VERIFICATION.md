...
初版目标来源：`XiuXianShop_First_Playable_OneShot.md`；当前目标为 `xianxia_pawnshop_next8h_codex_v3.md`。本记录区分自动验证和人工试玩。

## 最新验收：v3 一天营业交易切片（2026-09-08）

- 实际编译通过；Edit Mode **47/47**、Play Mode **10/10**，均无失败或跳过。完整名称、状态和耗时保存在 [V3_TEST_RESULTS.json](V3_TEST_RESULTS.json)，不是仅发现测试列表或编译成功。
- G-05 的仓库20/柜台23、正负修正相加得17、购买历史16、多件总价和预算/实际收支一致，由新增 ShopPricingTests 及实际 UGUI 自动 Play 测试覆盖。标签变化无需点击其他按钮便刷新显示与确认状态。
- 自动 Play 使用 Input System 虚拟鼠标和键盘：完整五位营业日含买家和卖家、购买历史、收入/支出/余额及睡觉后的日期和库存；另有20件清单滚动、总价与结算滚动位置恢复测试。原有18报价案例由独立零售加价0夹具保留，不冒充正常默认报价。
- 基于已读取 FigJam 图像完成可交互界面骨架，实际查看 1280×800、1920×1080 画面并检查按钮屏内/射线命中。显示修复保留；临时 Game 视图尺寸恢复，未修改渲染/输入/Build 设置。
- 详细 M0–M4 逐项审查、五张实机截图、临时规则及确定性菜单入口见 [TRADING_SLICE_V3.md](TRADING_SLICE_V3.md)。最终状态以 DEVLOG 本轮记录为准。
- 之前显示排查实际执行0项的结果仍保留为历史缺口。本轮重新编译后的两套测试实际执行47和10项，不把0项解释为通过，也未声称修复了测试框架。
- 人工手感试玩、独立 Player 构建、其他平台与所有分辨率：**未测**。正式美术、市场事件、长期经济及炼丹炉迁移不在本轮范围。

## 历史验收：每日五位随机顾客与离散资金档位（2026-09-07）

以下保留当时证据；旧固定价格、客流、资金数组、固定178灵石结局及 Console 状态不能作为当前规格。最新价格以 v3 为准。
